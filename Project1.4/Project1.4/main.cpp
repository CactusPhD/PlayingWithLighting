...
const float SPEED_ROT_RATIO = 2*5.77*3.1415 / 360;
float xPos = 0;
float zPos = 0;
float xRot = 0;
float yRot = 0;
...
void display() {
	...
	glUniformMatrix4fv(gModelLocation, 1, GL_TRUE, &(perspective*cameraTranslate*centerCamera*bunTranslate*xRotation*yRotation*centerBunny)[0][0]);

	glDrawElements(GL_TRIANGLES, faces.size(), GL_UNSIGNED_INT, (void*)0);

	glUniformMatrix4fv(COTLocation, 1, GL_TRUE, &(cameraTranslate*centerCamera*xRotation*bunTranslate*yRotation*centerBunny)[0][0]);
...
void init() {
	...
	gModelLocation = glGetUniformLocation(shaderProagram, "gModel");
	COTLocation = glGetUniformLocation(shaderProagram, "COT");
	//gColorLocation = glGetUniformLocation(shaderProagram, "color");
	specularPowerLocation = glGetUniformLocation(shaderProagram, "specularPower");
	glUniform1f(specularPowerLocation, 300);


	/*wheelShader = loadShaders("Shaders/WheelShader.vert", "Shaders/WheelShader.frag");
	gTrimLocation = glGetUniformLocation(wheelShader, "trim");
	gWheelColorLocation = glGetUniformLocation(wheelShader, "color");
	glUseProgram(wheelShader);*/
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("This Bunny is Lit");
